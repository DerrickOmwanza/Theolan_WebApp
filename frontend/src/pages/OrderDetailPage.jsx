...
const STATUS_STEPS = ['quoted', 'confirmed', 'fabrication', 'ready', 'installed'];
    ...
    return (
      <div className="flex items-center gap-3 p-4 rounded-lg bg-red-500/10 border border-red-500/20">
        ...
        <div>
          ...
          <p className="text-silver-500 text-xs">This order has been cancelled</p>
        </div>
      </div>
    );
  }
          ...
          <div key={step} className="flex-1 flex items-center">
            ...
            <div className="flex flex-col items-center flex-shrink-0">
              ...
              <span className={`mt-1.5 text-xs whitespace-nowrap ${
                isCompleted ? 'text-green-400' : isCurrent ? 'text-cobalt-300 font-medium' : 'text-silver-500'
              }`}>
                {statusLabels[step]}
              </span>
            </div>
      ...
      {events.map((event, index) => (
        <div key={event.id} className="flex gap-4">
          ...
          <div className={`pb-6 ${index === events.length - 1 ? 'pb-0' : ''}`}>
            <p className={`text-sm font-medium ${event.is_current ? 'text-cobalt-300' : 'text-silver-300'}`}>
              {event.title}
            </p>
            {event.description && (
              <p className="text-xs text-silver-500 mt-0.5">{event.description}</p>
            )}
      ...
      const poll = async () => {
        try {
          ...
          if (attempts < maxAttempts) {
            ...
            setStep('failed');
            setError('Payment verification timed out. Check your M-Pesa messages and refresh the page.');
          }
        } catch {
          attempts++;
          if (attempts < maxAttempts) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-charcoal-700 border border-charcoal-600 rounded-lg p-6 w-full max-w-md shadow-xl">
        ...
        {step === 'form' && (
          <>
            <h3 className="text-lg font-heading font-bold text-warmwhite mb-1">Pay for Order</h3>
            <p className="text-sm text-silver-400 mb-6">{order.reference_number} — {formatKes(remaining)} remaining</p>
        ...
        {step === 'processing' && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-heading font-bold text-warmwhite mb-2">Check Your Phone</h3>
            ...
            <p className="text-xs text-silver-500 mt-4">Waiting for payment confirmation...</p>
          </div>
        )}
        ...
        {step === 'success' && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-heading font-bold text-warmwhite mb-2">Payment Successful!</h3>
            <p className="text-sm text-silver-400 mb-6">
        ...
        {step === 'failed' && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-heading font-bold text-warmwhite mb-2">Payment Failed</h3>
...
export default function OrderDetailPage() {
  ...
  const remaining = order ? parseFloat(order.total_price_kes) - parseFloat(order.paid_amount_kes) : 0;
  const canPay = order && !isCancelled && remaining > 0 && order.status !== 'installed';
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        ...
        {canPay && (
          ...
          >
            ...
            Pay {remaining === parseFloat(order.total_price_kes) ? 'Deposit' : 'Balance'}
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Status Stepper */}
          <div className="card">
            <h3 className="text-sm font-medium text-silver-300 mb-4">Order Progress</h3>
            <StatusStepper currentStatus={order.status} isCancelled={isCancelled} />
          </div>

          {/* Timeline */}
          {order.timeline && order.timeline.length > 0 && (
            <div className="card">
              <h3 className="text-sm font-medium text-silver-300 mb-4">Timeline</h3>
          ...
          {order.dimensions_notes && (
            <div className="card">
              <h3 className="text-sm font-medium text-silver-300 mb-2">Dimensions & Notes</h3>
              <p className="text-sm text-silver-400 whitespace-pre-wrap">{order.dimensions_notes}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          {/* Payment Summary */}
          <div className="card">
            <h3 className="text-sm font-medium text-silver-300 mb-4">Payment Summary</h3>

            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-silver-400">Total Price</span>
                <span className="text-warmwhite font-medium">{formatKes(order.total_price_kes)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-silver-400">Paid</span>
                <span className="text-green-400">{formatKes(order.paid_amount_kes)}</span>
              </div>
              <div className="border-t border-charcoal-600 pt-3 flex justify-between text-sm">
                ...
                <span className={`font-medium ${remaining > 0 ? 'text-amber-400' : 'text-green-400'}`}>
                  {formatKes(remaining)}
                </span>
              </div>
            </div>
              ...
              <div className="flex justify-between text-xs text-silver-500 mb-1">
                ...
                <span>
                  {order.total_price_kes > 0
                    ? Math.round((parseFloat(order.paid_amount_kes) / parseFloat(order.total_price_kes)) * 100)
                    : 0}%
                </span>
              </div>
              <div className="h-2 bg-charcoal-600 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(
                      order.total_price_kes > 0
                        ? (parseFloat(order.paid_amount_kes) / parseFloat(order.total_price_kes)) * 100
                        : 0,
                      100
                    )}%`,
                  }}
          ...
          {/* Order Details */}
          <div className="card">
            <h3 className="text-sm font-medium text-silver-300 mb-4">Order Details</h3>
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-silver-300">{formatDate(order.created_at)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-silver-400">Last Updated</span>
                <span className="text-silver-300">{formatDate(order.updated_at)}</span>
              </div>
              {order.scheduled_installation_at && (
                <div className="flex justify-between">
                  <span className="text-silver-400">Installation</span>
                  <span className="text-cobalt-300">{formatDate(order.scheduled_installation_at)}</span>
                </div>
              )}
            </div>
          </div>
