      ...
      <section className="bg-charcoal-900 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-silver-300 max-w-2xl">
            Explore our completed projects across Nairobi and Kenya. Every installation reflects our commitment to precision and quality.
          </p>
        </div>
      </section>
      ...
      <section className="border-b border-charcoal-600 bg-charcoal-800 sticky top-16 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <span className="ml-auto text-sm text-silver-500">{total} project{total !== 1 ? 's' : ''}</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {isLoading ? (
          ...
          <div className="card text-center py-12">
            <p className="text-red-400">Failed to load gallery. Please try again.</p>
          </div>
        ) : photos.length === 0 ? (
          <div className="card text-center py-12">
            <p className="text-silver-400 mb-4">No projects found matching your filters.</p>
          ...
          <>
              ...
              {photos.map((photo, idx) => (
                ...
                >
                  ...
                  <div className="p-4 bg-charcoal-700">
                    {photo.project_name && (
                      <h3 className="text-warmwhite font-heading font-semibold mb-1">{photo.project_name}</h3>
                    )}
                    {photo.location && (
                    ...
                    <div className="flex gap-2 mt-2">
                      {photo.category && <span className="badge bg-charcoal-600 text-silver-300 capitalize text-xs">{photo.category.replace('_', ' ')}</span>}
                      {photo.finish && <span className="badge bg-charcoal-600 text-silver-300 capitalize text-xs">{photo.finish}</span>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-4 mt-10">
                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="btn-ghost text-sm disabled:opacity-30">Previous</button>
                <span className="text-sm text-silver-400">Page {page + 1} of {totalPages}</span>
                <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="btn-ghost text-sm disabled:opacity-30">Next</button>
              </div>
            )}
          </>
        )}
        ...
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={closeLightbox}>
          ...
          <div className="max-w-5xl max-h-[85vh] mx-4" onClick={(e) => e.stopPropagation()}>
            {photos[lightboxIndex].image_url ? (
              ...
              <div className="flex items-center justify-center h-[50vh] text-silver-500">No image available</div>
            )}
            <div className="mt-4 text-center">
              {photos[lightboxIndex].project_name && (
                <h3 className="text-warmwhite font-heading text-xl font-semibold">{photos[lightboxIndex].project_name}</h3>
              )}
              {photos[lightboxIndex].description && (
                <p className="text-silver-400 text-sm mt-1">{photos[lightboxIndex].description}</p>
              )}
              {photos[lightboxIndex].location && (
                <p className="text-silver-500 text-sm mt-1">{photos[lightboxIndex].location}</p>
              )}
            </div>
          </div>
        </div>
