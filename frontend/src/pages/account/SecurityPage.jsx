...
export default function SecurityPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-3xl font-heading font-bold text-warmwhite mb-2">Security</h2>
      <p className="text-silver-400 mb-8">Manage your account security</p>

      {/* Phone Verification Status */}
      <div className="card mb-6">
        <h3 className="text-lg font-heading font-semibold text-warmwhite mb-3">Phone Verification</h3>
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${user?.phone_verified ? 'bg-green-500' : 'bg-yellow-500'}`} />
      ...
      {/* Change Password */}
      <div className="card">
        <h3 className="text-lg font-heading font-semibold text-warmwhite mb-4">Change Password</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <button type="submit" disabled={submitting} className="btn-primary">
            {submitting ? <LoadingSpinner size="sm" className="text-white" /> : 'Change Password'}
          </button>
        </form>
      </div>
    </div>
