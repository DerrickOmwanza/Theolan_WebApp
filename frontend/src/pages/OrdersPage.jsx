...
const statusIcons = {
  quoted: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
  ...
  installed: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
...
export default function OrdersPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => orderApi.list(activeFilter !== 'all' ? { status: activeFilter } : {}),
    enabled: !!user,
  });
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h2 className="text-3xl font-heading font-bold text-warmwhite mb-2">My Orders</h2>
      ...
      {orders.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-silver-400 mb-2">
            {activeFilter !== 'all' ? `No ${statusLabels[activeFilter]?.toLowerCase()} orders` : 'No orders yet'}
          </p>
          ...
          {activeFilter === 'all' && (
            <Link to="/bookings" className="btn-primary inline-block">Book a Site Visit</Link>
          )}
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            const progressPercent = order.total_price_kes > 0
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-silver-500">
                      ...
                      {order.scheduled_installation_at && (
                        <span>Install: {formatDate(order.scheduled_installation_at)}</span>
                      )}
                    ...
                    <div className="w-32 h-2 bg-charcoal-600 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          progressPercent >= 100 ? 'bg-green-500' : progressPercent > 0 ? 'bg-cobalt' : 'bg-charcoal-600'
                        }`}
